...
function startRename() {
  const feed = feedsStore.feeds.find((f) => f.id === props.feedId);
  renameTitle.value = feed?.title || feed?.url || "";
...
async function renameFeed() {
  ...
  if (!title) return;
  await feedsStore.renameFeed(props.feedId, title);
...
<template>
  <Teleport to="body">
    <div
      ...
      :style="menuStyle"
    >
      <template v-if="!showLabelPicker && !showRename">
        <button
          @click="markAllRead"
        ...
        <button
          ...
          class="w-full text-left px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2"
        >
          <svg class="w-4 h-4 shrink-0"><use href="#icon-edit" /></svg>
          Rename
        </button>
      ...
      <template v-else-if="showRename">
        <div class="px-3 py-2">
          <div class="flex items-center justify-between mb-2">
            <button
              @click="backFromRename"
              class="text-xs text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200"
            >
              &larr; Back
            </button>
            <span class="text-xs font-semibold text-gray-500 dark:text-gray-400">Rename feed</span>
            <div class="w-4" />
          </div>
          <form @submit.prevent="renameFeed" class="flex gap-1">
            <input
              ref="renameInput"
              v-model="renameTitle"
