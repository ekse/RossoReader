...
const lastUpdate = computed(() => {
  const dates = feedsStore.feeds.map((f) => f.last_fetched_at).filter(Boolean) as string[];
...
<template>
  <div class="flex flex-col h-full">
    <div class="flex-1 p-4 overflow-y-auto">
      ...
      <router-link to="/unread" class="block px-3 py-1.5 rounded-md text-sm font-medium" :class="route.path === '/unread'
      ...
      <router-link to="/starred" class="block px-3 py-1.5 rounded-md text-sm font-medium mt-1" :class="route.path === '/starred'
        ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300'
        : 'text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700/50'
        ">
        Starred
      ...
      <router-link to="/settings" class="block px-3 py-1.5 rounded-md text-sm font-medium mt-1" :class="route.path === '/settings'
        ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300'
        : 'text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700/50'
        ">
        Settings
      ...
      <router-link v-if="auth.isAdmin" to="/admin" class="block px-3 py-1.5 rounded-md text-sm font-medium mt-1" :class="route.path === '/admin'
        ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300'
        : 'text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700/50'
        ">
        Administration
      ...
      <div class="mt-3">
        <div class="px-3 flex items-center justify-between">
          ...
          <button @click="feedsStore.filterUnreadOnly = !feedsStore.filterUnreadOnly" :title="feedsStore.filterUnreadOnly ? 'Show all feeds' : 'Show only feeds with unread items'
            " class="w-5 h-5 flex items-center justify-center rounded transition-colors" :class="feedsStore.filterUnreadOnly
        ...
        <div class="mt-2 space-y-1">
          <button v-for="feed in visibleFeeds" :key="feed.id" @click="selectFeed(feed.id)"
            class="w-full text-left px-3 py-2 rounded-md text-sm" :class="route.params.id === String(feed.id)
              ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300'
              : 'text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700/50'
              ">
    ...
    <div v-if="auth.user"
      ...
      <router-link to="/settings"
        ...
        <span v-if="auth.isAdmin"
          class="px-1.5 py-0.5 text-[10px] font-medium rounded bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">admin</span>
      </router-link>
