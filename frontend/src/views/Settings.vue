...
<template>
  <div class="max-w-3xl mx-auto px-4 py-6 md:px-6 md:py-8">
    ...
    <section class="mt-8">
      ...
      <div class="space-y-2">
        <div v-for="feed in feedsStore.feeds" :key="feed.id"
          ...
          <div class="min-w-0 flex-1">
            <p class="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">
              {{ feed.title || feed.url }}
              <span v-if="feed.last_fetch_error" @click.stop="toggleError(feed.id)"
                class="cursor-pointer mr-1.5 select-none" title="Show error">⚠️</span>
            </p>
            <p class="text-xs text-gray-500 dark:text-gray-400 truncate">{{ feed.url }}</p>
            <p v-if="feed.last_fetch_error && expandedErrors.has(feed.id)"
              class="mt-1.5 text-xs text-red-600 dark:text-red-400 break-words">
              <b>Last error:</b> {{ feed.last_fetch_error }}
            </p>
          </div>
          <button @click="feedsStore.removeFeed(feed.id)"
    ...
    <section class="mt-10">
      ...
      <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
        ...
        <span v-if="auth.isAdmin"
          class="ml-2 px-2 py-0.5 text-xs font-medium rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">admin</span>
      </p>
      ...
      <div class="mt-4 max-w-md">
        ...
        <div class="mt-2 space-y-3">
          ...
          <div class="relative">
            <input v-model="newPassword" :type="showNewPassword ? 'text' : 'password'" placeholder="New password"
    ...
    <section v-if="auth.isAdmin" class="mt-10">
          ...
          <div v-for="u in users" :key="u.id"
            ...
            <div class="flex items-center gap-2">
              ...
              <span v-if="u.is_admin"
                class="px-2 py-0.5 text-xs font-medium rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">admin</span>
            </div>
            <button v-if="canDeleteUser(u)" @click="deleteUser(u)"
      ...
      <div class="mt-6 max-w-md">
        ...
        <div class="mt-2 space-y-3">
          <input v-model="newUserUsername" type="text" placeholder="Username" autocomplete="username"
          ...
          <div class="relative">
            <input v-model="newUserPassword" :type="showNewUserPassword ? 'text' : 'password'" placeholder="Password"
              autocomplete="new-password"
              class="w-full px-3 py-2 pr-10 border border-gray-300 dark:border-gray-600 rounded-md text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-blue-500 focus:border-blue-500" />
